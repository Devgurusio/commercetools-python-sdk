...
class BaseActions:
  ...
  def get_actions(cls, old_obj: BaseModel, new_obj: BaseModel):
    ...
    if (diff.__contains__('attribute_added')):
      actions.extend(cls._regular_attribute_actions(diff['attribute_added'], new_obj, old_obj))
    if (diff.__contains__('type_changes')):
      actions.extend(cls._regular_attribute_actions(diff['type_changes'], new_obj, old_obj))
    if (diff.__contains__('iterable_item_added')):
      actions.extend(cls._iterable_attribute_add_actions(diff['iterable_item_added'], new_obj, old_obj))
    if (diff.__contains__('iterable_item_removed')):
      actions.extend(cls._iterable_attribute_remove_actions(diff['iterable_item_removed'], new_obj, old_obj))
    if (diff.__contains__('values_changed')):
      actions.extend(cls._regular_attribute_actions(diff['values_changed'], new_obj, old_obj))
      actions.extend(cls._iterable_attribute_update_actions(diff['values_changed'], new_obj, old_obj))
    if (diff.__contains__('dictionary_item_added')):
      actions.extend(cls._diccionary_attribute_add_actions(diff['dictionary_item_added'], new_obj, old_obj))
    if diff.__contains__('dictionary_item_removed'):
      actions.extend(cls._diccionary_attribute_remove_actions(diff['dictionary_item_removed'], new_obj, old_obj))
