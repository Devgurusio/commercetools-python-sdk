...
class TypeActions(BaseActions):
    ...
    def _regular_attribute_actions(cls, diff: dict, obj, old_obj=None):
        ...
        for root_attr in diff:
            ...
            if attr == 'key':
                ...
                if field == '_name':
                    actions.append({'action': 'removeFieldDefinition', 'fieldName': old_obj.fieldDefinitions[int(
                        re.findall(r'[\d+]', attr)[0])].name})
                    actions.append({'action': 'addFieldDefinition', 'fieldDefinition': obj.fieldDefinitions[int(
                        re.findall(r'[\d+]', attr)[0])].toDict()})
        return actions
                ...
                try:
                    ...
                    if field.__contains__("type['values']"):
                        if obj.fieldDefinitions[int(re.findall(r'[\d+]', attr)[0])].type['name'] == 'Enum':
                            actions.append({'action': 'addEnumValue', 'fieldName': obj.fieldDefinitions[int(
                                re.findall(r'[\d+]', attr)[0])].name, 'value': diff[root_attr]})
                        elif obj.fieldDefinitions[int(re.findall(r'[\d+]', attr)[0])].type['name'] == 'LocalizedEnum':
                            actions.append({'action': 'addLocalizedEnumValue', 'fieldName': obj.fieldDefinitions[int(
                                re.findall(r'[\d+]', attr)[0])].name, 'value': diff[root_attr]})
                except IndexError:
                    actions.append({'action': 'addFieldDefinition', 'fieldDefinition': obj.fieldDefinitions[int(
                        re.findall(r'[\d+]', attr)[0])].toDict()})
    ...
    def _iterable_attribute_remove_actions(cls, diff: dict, obj, old_obj=None):
        ...
        for root_attr in diff:
            attr = root_attr.split('.')[1]
            if attr.__contains__('fieldDefinitions'):
                actions.append({'action': 'removeFieldDefinition',
                                'fieldName': diff[root_attr].name})
        return actions
        ...
        for root_attr in diff:
            ...
            if attr.startswith('name'):
                ...
                if field.startswith('label'):
                    actions.append({'action': 'changeLabel', 'fieldName': obj.fieldDefinitions[int(re.findall(
                        r'[\d+]', attr)[0])].name, 'label': obj.fieldDefinitions[int(re.findall(r'[\d+]', attr)[0])].label})
        return actions
