...
class ZoneActions(BaseActions):
  ...
  def _iterable_attribute_add_actions(cls, diff: dict, obj):
    ...
    for root_attr in diff:
        ...
        if (attr.__contains__('locations')):
          actions.append({'action': 'addLocation', 'location': diff[root_attr].__dict__ })
    return actions
  ...
  def _iterable_attribute_update_actions(cls, diff: dict, obj, old_obj):
    ...
    for root_attr in diff:
        attr = root_attr.split('.')[1]
        if (attr.__contains__('locations')):
          actions.append({'action': 'removeLocation', 'location': old_obj.locations[int(re.findall(r'[\d+]', attr)[0])].__dict__})
          actions.append({'action': 'addLocation', 'location': obj.locations[int(re.findall(r'[\d+]', attr)[0])].__dict__})
    return actions
  ...
  def _iterable_attribute_remove_actions(cls, diff: dict, obj):
    ...
    for root_attr in diff:
        attr = root_attr.split('.')[1]
        if (attr.__contains__('locations')):
          actions.append({'action': 'removeLocation', 'location': diff[root_attr].__dict__ })
    return actions