...
class StringMatches(object):
  ...
  def __call__(self, func):
    def authorize_and_call(obj, value, *args, **kwargs):
      if value != None:
        if self.min_len != None and value.__len__() < self.min_len:
          raise ValueError('Value length is less than %s' % self.min_len)
        if self.max_len != None and value.__len__() > self.max_len:
          raise ValueError('Value length is higher than %s' % self.max_len)
        if self.regular_expression_object.fullmatch(value) == None:
          raise ValueError('Value doesn\'t match pattern "%s" with flags %s' % (self.pattern, self.regular_expression_object.flags))
      return func(obj, value, *args, **kwargs)
